"""
Edge-IIoT Dataset implementation for TRUST_MCNet.
"""
...
@register("edge_iiot")
class EdgeIIoTDataset:
    """
    Edge-IIoT dataset loader for federated learning.
    
    Provides standardized interface for loading Edge-IIoT industrial IoT data
    ...
    def _load_data(self):
        """Load and preprocess Edge-IIoT data."""
        data_path = os.path.join(self.data_root, "IoT_Datasets", "Edge_IIoT")
        
        # Check if data exists, create synthetic data if not
        if not os.path.exists(data_path):
            print(f"Warning: Edge-IIoT data not found at {data_path}. Creating synthetic data.")
        ...
        try:
            ...
            print(f"Error loading Edge-IIoT data: {e}. Creating synthetic data.")
            self._create_synthetic_data()
    ...
    def _create_synthetic_data(self):
        ...
        # Generate synthetic industrial sensor features
        n_samples = 4000
        n_features = 61  # Common number of features in industrial IoT datasets
        
        # Normal operations (70%)
        normal_samples = int(0.7 * n_samples)
        normal_data = np.random.normal(0, 0.8, (normal_samples, n_features))
        
        # DDoS attacks (15%)
        ddos_samples = int(0.15 * n_samples)
        ddos_data = np.random.normal(3, 2, (ddos_samples, n_features))
        
        # DoS attacks (10%)
        dos_samples = int(0.1 * n_samples)
        dos_data = np.random.normal(-2, 1.5, (dos_samples, n_features))
        
        # Other attacks (5%)
        other_samples = n_samples - normal_samples - ddos_samples - dos_samples
        other_data = np.random.normal(1.5, 2.5, (other_samples, n_features))
        
        # Combine data
        X = np.vstack([normal_data, ddos_data, dos_data, other_data])
        y = np.hstack([
            np.zeros(normal_samples),  # Normal: 0
            np.ones(ddos_samples),     # DDoS: 1
            np.full(dos_samples, 2),   # DoS: 2
            np.full(other_samples, 3)  # Other: 3
        ])
